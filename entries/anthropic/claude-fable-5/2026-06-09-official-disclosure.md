...
# Anthropic Claude Fable 5 — web-app

## Provenance

This is an `official-disclosure` capture. Anthropic publishes the system prompts for its
consumer Claude apps (web, mobile, desktop) in its own release notes, and updates the page
when a prompt changes. The text below is reproduced verbatim from that page.

- **Source:** Anthropic release notes, "System Prompts" — https://platform.claude.com/docs/en/release-notes/system-prompts
- **Listed model / date:** "Claude Fable 5" dated **June 9, 2026**, the most recent entry on the page at the time of access.
- **Accessed:** 2026-06-14. (The original page URL `docs.anthropic.com/en/release-notes/system-prompts` 301-redirects to the `platform.claude.com` host above.)
- **Scope disclosed — read this carefully:** Anthropic discloses only the natural-language
  *behavioral* portion of the system prompt (the `<claude_behavior>` block: product info,
  refusal handling, tone, wellbeing, etc.). The disclosure does **not** include tool/function
  definitions, the contents of runtime-injected reminders (e.g. `long_conversation_reminder`),
  user preferences, style settings, or any per-session context. The published text also names
  reminder types it does not contain the bodies of. For this reason `partial` is set to `true`:
  this is the complete *disclosed* prompt, but not the complete *running* context. Per
  CONTRIBUTING.md, an official disclosure may not match what the model is actually run with,
  and that gap is flagged here rather than hidden.
...
**High**, with one important scope caveat.

The text is high-confidence because it comes directly from Anthropic's own published release notes — there is no elicitation or third-party transcription between the vendor and this entry. Anthropic maintains this page specifically to disclose these prompts.

The caveat is the same one recorded under Provenance: this is the disclosed prompt, not the full running context. The disclosure is limited to the `<claude_behavior>` block and omits tool definitions, runtime-injected reminder bodies, user preferences, style settings, and per-session context. Treat the absence of those in this entry as a property of the disclosure, not evidence that Claude Fable 5 runs without them. Hence `partial: true`.

Two verbatim artifacts worth noting, preserved exactly as published rather than "corrected":
- The prompt opens with `<claude_behavior>` and the product-information section is closed with a `</product_information>` tag that has no matching opening tag in the disclosed text.
- The `{{currentDateTime}}` token in the knowledge-cutoff section is a template placeholder, substituted at runtime; it is left intact here because that is how it appears in the disclosure.
