...
# Anthropic Claude Opus 4.8 — web-app
...
- **Listed model / date:** "Claude Opus 4.8" dated **May 28, 2026**.
...
- **Scope disclosed:** only the natural-language `<claude_behavior>` block. Tool/function
  ...
  per-session context are not part of the disclosure. `partial` is therefore `true`: this is
  the complete *disclosed* prompt, not the complete *running* context.
...
Scope caveat as recorded under Provenance: the disclosure covers only `<claude_behavior>`
(plus the trailing `<tone_preference>` block) and omits tool definitions, runtime reminder
bodies, user preferences, and per-session context, so `partial: true`. The `{{currentDateTime}}`
token in the knowledge-cutoff section is a runtime template placeholder, preserved as published.
As with the Fable 5 capture, the product-information section is closed with a
`</product_information>` tag that has no matching opening tag in the disclosed text; preserved
