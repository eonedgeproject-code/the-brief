...
# Anthropic Claude Opus 4.6 — web-app

## Provenance

This is an `official-disclosure` capture, reproduced from Anthropic's published release-notes
"System Prompts" page.

- **Source:** https://platform.claude.com/docs/en/release-notes/system-prompts
- **Listed model / date:** "Claude Opus 4.6" dated **February 5, 2026** — the oldest entry on the page at time of access.
- **Accessed:** 2026-06-14 (original `docs.anthropic.com` URL 301-redirects to the `platform.claude.com` host).
- **Scope disclosed:** only the natural-language `<claude_behavior>` block. Tool/function
  definitions, runtime-injected reminder bodies, user preferences, style settings, and
  per-session context are not part of the disclosure. `partial` is therefore `true`.
- **Extraction caveat — read the Confidence section.** This capture was extracted via an
  intermediary fetch step that flagged the possibility of source-side truncation for this
  specific entry. See below.
...
**Medium** — downgraded from the `high` of the other official-disclosure captures, for one
specific reason.

The text comes from the same official Anthropic release-notes page, so the *source* is as
authoritative as the others. However, this entry was retrieved through an intermediary fetch
step that explicitly flagged the possibility of **source-side truncation** for the February 5,
2026 Opus 4.6 entry (a "[Content truncated due to length...]" marker was reported). The captured
text above is internally complete and well-formed — it opens with `<claude_behavior>`, every
expected section is present, and it closes all tags — so there is no *visible* gap. But because
the extraction tool raised the flag, this entry cannot be asserted at the same confidence as the
others without a clean re-fetch directly against the source.

Recommended follow-up: re-capture this entry directly from
`https://platform.claude.com/docs/en/release-notes/system-prompts` and, if the text matches,
upgrade `confidence` to `high`; if it differs, record a corrected capture and preserve this one.

Scope caveat as for the other captures: disclosure covers only `<claude_behavior>`; tool
definitions, runtime reminder bodies, user preferences, and per-session context are not included,
so `partial: true`. The `{{currentDateTime}}` token is a runtime placeholder, preserved as
published. The product-information section is closed with a `</product_information>` tag with no
matching opening tag; preserved exactly.
