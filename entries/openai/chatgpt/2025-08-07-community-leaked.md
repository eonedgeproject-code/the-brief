...
sources:
  - url: https://github.com/elder-plinius/CL4R1T4S/blob/main/OPENAI/ChatGPT5-08-07-2025.mkd
    author: elder-plinius (CL4R1T4S)
    original_date: 2025-08-07
---

# OpenAI ChatGPT (GPT-5) — web-app

## Provenance

This is a `community-leaked` capture. The text was **not** obtained by this archive; it is
transcribed from a third-party aggregator of leaked prompts.

- **Cited source:** CL4R1T4S, file `OPENAI/ChatGPT5-08-07-2025.mkd` — https://github.com/elder-plinius/CL4R1T4S
- **Version pin:** CL4R1T4S repository HEAD commit `dc626fed52b06d687cdc812d51090c95ed03d575` (short `dc626fe`), latest commit dated 2026-06-09.
- **Original capture date:** the filename and the prompt's own `Current date: 2025-08-07` line indicate the leak dates to ~2025-08-07; that is used as `captured`.
- **Accessed:** 2026-06-14, fetched as raw bytes via `curl` to preserve the text exactly.
- **Trust limits — read the Confidence section.** CL4R1T4S aggregates leaked prompts but does not document a per-file acquisition method (who extracted it, how, or whether it reproduced). This archive cannot independently verify the text against the live product.

## Prompt

Reproduced verbatim from the cited CL4R1T4S file, including its leading metadata header.
...
**Low** — and deliberately so. This rating exists to demonstrate exactly the kind of distinction
this archive's method tagging is for.

Why low, not medium:
- **No per-file acquisition story.** CL4R1T4S aggregates leaked prompts but does not document, for this file, who extracted it, by what technique, on what surface, or whether it reproduced. Per this archive's own CONTRIBUTING rules, a cited leak whose original capture method is undocumented is weak evidence.
- **We cannot independently verify it.** This archive did not extract the text from ChatGPT and has not confirmed it against the live product. We can confirm only what the CL4R1T4S file says, pinned to commit `dc626fe`.
- **Plausibility is not verification.** The content is internally consistent and matches the known shape of ChatGPT system prompts (memory/`bio`, `automations`, `canmore`, `file_search`, `python`, `web`, etc.), which raises plausibility — but plausible-looking text is exactly what a fabricated or partially-edited leak would also look like.

`partial` is marked `false` because the file reproduces a self-contained prompt ending in "End
of system prompt." — but note that the *completeness of the underlying leak itself* is
unverifiable; a leak can be truncated or lightly edited before publication without any visible
sign.

## Notes

**Trust limits for the `community-leaked` method.** A community-leaked entry is only as trustworthy
as (a) the original extractor's method and (b) the aggregator's fidelity, and here neither is
documented per-file. Treat this capture as *an artifact published by CL4R1T4S that claims to be
ChatGPT's GPT-5 system prompt*, not as confirmed ground truth. The honest contrast for a reader:
the xAI entry in this archive is an `official-disclosure` of an actual system prompt (high
confidence); this is a `community-leaked` claim about one (low confidence). Same subject category
— a real running system prompt — but very different evidentiary weight, and the method tag is how
the archive makes that difference legible at a glance.

To strengthen this entry, someone would need to either (1) reproduce the extraction directly and
re-file it as `self-extracted`, or (2) surface the original leak with a documented acquisition
method.
