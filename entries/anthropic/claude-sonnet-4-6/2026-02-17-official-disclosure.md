...
# Anthropic Claude Sonnet 4.6 — web-app

## Provenance

This is an `official-disclosure` capture, reproduced verbatim from Anthropic's published
release-notes "System Prompts" page.

- **Source:** https://platform.claude.com/docs/en/release-notes/system-prompts
- **Listed model / date:** "Claude Sonnet 4.6" dated **February 17, 2026**.
...
Note the older knowledge cutoff (August 2025) in this capture relative to the Opus 4.7/4.8 and
Fable 5 captures (January 2026) — a genuine difference between the models, preserved as published.
...
**High.** Reproduced verbatim from Anthropic's own release notes; no elicitation or third-party
transcription is involved.

Scope caveat as recorded under Provenance: the disclosure covers only `<claude_behavior>` and
omits tool definitions, runtime reminder bodies, user preferences, and per-session context, so
`partial: true`. The `{{currentDateTime}}` token is a runtime template placeholder, preserved as
published. The product-information section is closed with a `</product_information>` tag with no
matching opening tag in the disclosed text; preserved exactly. Note that in this capture the
section ordering differs from the Opus captures (`user_wellbeing` appears after
`responding_to_mistakes_and_criticism`); preserved as published.
