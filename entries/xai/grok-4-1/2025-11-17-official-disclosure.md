...
sources:
  - url: https://github.com/xai-org/grok-prompts/blob/main/grok4p1_thinking_system_turn_prompt_v2.j2
    author: xAI
    original_date: 2025-11-17
---

# xAI Grok 4.1 (thinking) — web-app

## Provenance

This is an `official-disclosure` capture, and unlike the OpenAI Model Spec and Google policy
entries it is an **actual system prompt**, not a high-level policy. xAI publishes the real
system prompts used for the Grok chat assistant and `@grok` on X in a public, self-described
official repository.

- **Source repo:** https://github.com/xai-org/grok-prompts (org: `xai-org`, license AGPL-3.0). The README states the repo holds "the system prompts that we use for the Grok chat assistant and various product features across X and grok.com," updated regularly.
- **Specific file captured:** `grok4p1_thinking_system_turn_prompt_v2.j2` — the system-turn prompt for the Grok 4.1 *thinking* variant.
- **Version pin:** repository HEAD commit `a7c186f5ccac95875c0041aed60398f6ecb6d6c7`, dated **2025-11-17**. Captured date reflects that commit.
- **Retrieval:** fetched as raw bytes from `raw.githubusercontent.com` via `curl` on 2026-06-14, deliberately bypassing any summarizing layer to preserve the text exactly. The `.j2` extension indicates a Jinja2 template; the `{% ... %}`, `{{ ... }}`, and `{#- ... -#}` tags below are runtime template directives, preserved verbatim.
...
**High.** Reproduced byte-for-byte from xAI's own public repository via raw fetch; no
elicitation or third-party transcription is involved, and the source self-identifies as the
official set of prompts.

`partial` is `false`: the template file is captured in its entirety. Note however that this is
the *thinking-variant* system-turn prompt; the same repo carries sibling files for other Grok
variants (e.g. `grok4p1_non_thinking_*`, `grok_4_mini_system_prompt.txt`, the `@grok` bot, and
separate safety prompts), each of which would be its own capture. The `{{custom_personality}}`
and `{{user_info}}` placeholders are filled at runtime, so a live Grok session contains those
substituted values in addition to this text.

## Notes

This is the rare official disclosure that *is* the actual running system prompt rather than a
policy or specification — a useful contrast in this archive with the OpenAI Model Spec and
Google Prohibited Use Policy entries, which are explicitly not system prompts.
